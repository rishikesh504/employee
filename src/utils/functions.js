...
export function validateUser (user, step)  {
  ...
  if (step === 0) {
    ...
    //   errors.experience = "Please add a experience";
    // }
    var nullexp = user.experienceList.find((experience) => experience.company == ''|| experience.role == '');      //checking if experiece fields are exist
    if (nullexp) {
      errors.experience = "Please fill all current fields"
    } 
  }
  else if (step === 3)  {
    if (!user.educationList.length > 0) {
      errors.education = "Please add atleast one education background";
    }
    var nullexp = user.educationList.find((education) => education.college == ''|| education.percentage == '');      //checking if experiece fields are exist
    if (nullexp) {
      errors.education = "Please fill all current fields"
    }  
